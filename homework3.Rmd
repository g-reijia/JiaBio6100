...
First, I removed additional characters by using the following method:

- Find:(?<=\n).*(?=\n[^\n]*?\n)
- Replace:

However, this gets rid of the snake_case in the "id" row, so I put those back in by using the following method:

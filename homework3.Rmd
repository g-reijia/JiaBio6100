...
```         
Find the all of the characters, numbers, and commas within the string of text and remove them, and 
...
```         
Find the first (1) and second (2) character/word (1) (2) along with the commma between them, then find 
...
First, I wanted to remove additional characters and did that using the following method:
- Find:[^,/.a-zA-Z\d\s:]
- Replace: (don't replace anything)

However, I noticed that this removed the underscores in the first row. I tried to isolate the first row and only remove characters from lines 2-21. I spent about 5 hours trying to figure this out with no luck, but here is the code I used to isolate the first row from the remianing lines.
